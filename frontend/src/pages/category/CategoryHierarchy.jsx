...
export default function CategoryHierarchy({ onBack, onCreate, onEdit }) {
  ...
  const [activeView, setActiveView] = useState('tree'); // 'list' | 'tree' | 'analytics'
    ...
    <div className="p-6">
      ...
      <div className="flex items-center gap-2 mb-5">
        {[
          ...
          <button
            key={tab.key}
            onClick={() => { if (tab.key === 'list') onBack(); else setActiveView(tab.key); }}
      ...
      <div className="grid grid-cols-5 gap-4" style={{ minHeight: 500 }}>
        ...
        <div className="col-span-2 bg-white rounded-xl border border-slate-200 p-4">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-sm font-semibold text-slate-700">Category Tree</h3>
          </div>
          ...
          {!selected ? (
            ...
            <>
              ...
              <div className="grid grid-cols-3 gap-4 mb-5 text-sm">
                ...
                <div>
                  <p className="text-xs text-slate-400 uppercase tracking-wider mb-1">Sort Order</p>
                  <p className="font-medium text-slate-700">{String(selected.sortOrder || 1).padStart(2, '0')}</p>
                </div>
              </div>
