...
const GoodsReceivingPageSchema = new mongoose.Schema({
  ...
  receiptNumber: { type: String, unique: true }, // e.g. GR-2026-0842, auto-generated
      ...
      items: {
        ...
        validate: [(arr) => arr.length > 0, 'At least one received item is required.'],
      },
      ...
      status: {
        type: String,
        enum: ['DRAFT', 'PENDING_VERIFICATION', 'DISCREPANCY', 'VERIFIED', 'REJECTED'],
...
GoodsReceivingPageSchema.pre('save', async function (next) {
  ...
  this.discrepancyCount = this.items.filter((i) => i.itemStatus !== 'MATCHED').length;
...
GoodsReceivingPageSchema.index({ receiptNumber: 1 });
